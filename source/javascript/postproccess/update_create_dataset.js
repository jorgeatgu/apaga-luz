...
const json_today_prices = await readJSON('/public/data/todays_price.json');
const json_all_prices_yesterday = await readJSON('all_prices.json');
...
await writeJSON('all_prices.json', json_all_prices)
...
const new_file_by_day = 'group_prices_by_day.json';
const new_file_by_month = 'group_prices_by_month.json';
await writeJSON(new_file_by_day, group_data_by_day)
await writeJSON(new_file_by_month, group_prices_by_month)

/*function get_n_dates_past_days(n_days) {
  return [...Array(n_days)].map((_, index) => {
