document.addEventListener('DOMContentLoaded', function () {
  ...
  if (
    pageContent.includes('discriminación horaria') ||
    pageContent.includes('discriminacion horaria')
  ) {
    ...
    pageContent.includes('tarifa valle') ||
    pageContent.includes('hora valle')
  ) {
    ...
    pageContent.includes('tarifa fija') ||
    pageContent.includes('precio fijo')
  ) {
  ...
  const timeSelectors = document.querySelectorAll(
    '.time-period-selector, .hour-selector, [data-period]'
  );
  if (timeSelectors.length > 0) {
    timeSelectors.forEach(selector => {
      selector.addEventListener('click', function () {
        let timeSegment =
          this.dataset.period || this.textContent.trim().toLowerCase();
  ...
  const priceTableHeaders = document.querySelectorAll(
    'table th, .price-table th, .tariff-table th'
  );
  ...
  const tableFilters = document.querySelectorAll(
    '.table-filter, select, [data-filter]'
  );
